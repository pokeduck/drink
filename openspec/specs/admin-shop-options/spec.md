...
定義 Admin 後台店家層級「飲料選項啟用過濾」的契約：以 `ShopEnabledSugar` / `ShopEnabledIce` / `ShopEnabledTopping` / `ShopEnabledSize` 四張啟用表記錄每間店家啟用的 Sugar / Ice / Topping / Size，未啟用視為停用（default disable）；透過 dry-run preview 揭露停用對既有品項的連鎖影響，並提供整批 delete-then-insert + cascade 移除 ShopMenuItemXxx 的更新流程。本 capability 同時將店家內排序集中為啟用表的 `Sort` 欄位作為唯一來源，避免與 override 表的 sort 重疊。
