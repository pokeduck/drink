...
定義 Client 前台會員認證契約：包含 Email/Password 註冊與驗證信流程、登入、Refresh Token Rotation、登出，以及驗證信寄送的抽象介面（dev 階段以 Serilog Mock）。所有 token 採 JWT，Refresh Token 採 rotation 並偵測重複使用攻擊。
