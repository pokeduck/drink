...
### Requirement: 刪除分類
系統 SHALL 提供 `DELETE /api/admin/shops/{shopId}/categories/{categoryId}` 端點。刪除分類時底下所有品項 SHALL 一併 soft delete。
...
### Requirement: 刪除品項
系統 SHALL 提供 `DELETE /api/admin/shops/{shopId}/categories/{categoryId}/items/{itemId}` 端點，執行 soft delete。
