...
前台訂單管理 capability:涵蓋使用者在 Client app 發起 / 編輯 / 取消揪團、加入下單、編輯與刪除自己的飲料項目,並在寫入時做價格快照(`ShopMenuItemSize` / `ShopSugarOverride` / `ShopToppingOverride`)。權限規則為「Initiator 可改自家揪團與全部飲料;Filler 僅可改自己下的飲料」,沿用 admin-order capability 的 Entity 與 ErrorCodes 4-05-XX。
