...
const fetchList = async () => {
  startListLoading()
  try {
    const { data: res } = await api.GET('/api/admin/sizes', {
      params: {
        query: {
          page: page.value,
...
const handleSelectionChange = (rows: Size[]) => {
  selectedRows.value = rows
}

// ---- 刪除 ----
const handleDelete = async (row: Size) => {
  try {
    ...
    startLoading()
    const { error } = await api.DELETE('/api/admin/sizes/{id}', {
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    startLoading()
    const { error } = await api.DELETE('/api/admin/sizes/batch', {
...
const handleSaveSort = async () => {
  ...
  const { error } = await api.PUT('/api/admin/sizes/sort', { body: { items } })
  await stopLoading()
    ...
    <el-card shadow="never" v-loading="loading">
      ...
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          ...
          <el-button v-if="can(MENU.Size, 'create')" type="primary" icon="Plus" @click="router.push('/drink-option/size/create')">
            新增容量
          </el-button>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column label="排序" width="150">
          <template #default="{ row }">
            <el-input-number v-if="can(MENU.Size, 'update')" v-model="row.sort" :min="0" :precision="0" style="width: 120px" />
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template #default="{ row }">
            <el-button v-if="can(MENU.Size, 'update')" size="small" @click="router.push(`/drink-option/size/${row.id}/edit`)">編輯</el-button>
