...
const fetchData = async () => {
  ...
  const { data: res, error } = await api.GET('/api/admin/sugars/{id}', {
    params: { path: { id } },
  })
  fetchLoading.value = false
  if (error) {
    handleError(error, '載入甜度資料失敗')
    router.push('/drink-option/sugar/list')
    return
  }
  const item = res!.data!
  form.name = item.name!
  form.default_price = item.default_price!
  form.sort = item.sort!
}
...
<template>
  <div>
    ...
    <el-page-header title="返回上一頁" @back="router.push('/drink-option/sugar/list')">
      <template #content>編輯甜度</template>
    </el-page-header>
