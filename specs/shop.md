...
- 單一 API 回傳該店家菜單所需的**所有資料**，前端不需額外呼叫飲料選項 API
- `definitions` 區塊包含該菜單依賴的所有 Size、Sugar、Ice、Topping 定義
  - Sugar / Topping 的 price 和 sort 回傳**實際生效值**（有覆寫用覆寫，無覆寫用全域預設）
  - 僅回傳該店家菜單有使用到的選項（非全域全部）
- `categories` 區塊的品項僅回傳 ID 陣列，前端透過 `definitions` 對應顯示
- Response:
```json
{
  "data": {
    ...
    "definitions": {
      ...
      "sugars": [
        { "id": 1, "name": "正常糖", "price": 0, "sort": 1 },
        { "id": 6, "name": "加蜂蜜", "price": 10, "sort": 6 }
      ],
      "ices": [
      ...
      "toppings": [
        { "id": 1, "name": "珍珠", "price": 15, "sort": 1 },
        { "id": 2, "name": "椰果", "price": 10, "sort": 2 }
      ]
    },
    "categories": [
      {
...
- [ ] `GET /api/user/shops/{shopId}/menu` 回傳 definitions（該菜單依賴的 Size/Sugar/Ice/Topping 定義，含實際生效價格）+ categories（品項以 ID 引用 definitions），支援 keyword 模糊搜尋品項名稱
- [ ] 下架店家回傳 404
